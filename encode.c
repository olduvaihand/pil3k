...
typedef struct {
    PyObject_HEAD
    int (*encode)(Imaging im, ImagingCodecState state, UINT8* buffer,
            int bytes);
...
PyImaging_EncoderNew(int contextsize)
{
    ...
    ImagingEncoderType.tp_new = &PyType_GenericNew;
...
PyImaging_ZipEncoderNew(PyObject* self, PyObject* args)
{
    ...
    if (!PyArg_ParseTuple(args, "ss|iy#", &mode, &rawmode, &optimize,
...
PyImaging_JpegEncoderNew(PyObject* self, PyObject* args)
{
    ...
    if (!PyArg_ParseTuple(args, "ss|iiiiiiiiy#", &mode, &rawmode, &quality,
           &progressive, &smooth, &optimize, &streamtype, &xdpi, &ydpi,
           &subsampling, &extra, &extra_size))
        return NULL;
