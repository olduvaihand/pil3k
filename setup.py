...
DOWNLOAD_URL = "http://effbot.org/downloads/{0}-{1}.tar.gz" # name, version
...
VERSION = find_version("pil3k/Image.py")
...
class pil_build_ext(build_ext):
    ...
    def build_extensions(self):
        ...
        if sys.platform == "cygwin":
            ...
            add_directory(library_dirs, os.path.join(
                "/usr/lib", "python{0}".format(sys.version[:3]), "config"
                ))
    ...
    def summary_report(self, feature, unsafe_zlib):
        ...
        print("-" * 68)
        print("pil3k", VERSION, "SETUP SUMMARY")
        print("-" * 68)
