...
def _save(im, fp, filename, check=0):
    ...
    if im.mode == "P":
        ...
        raise IOError("cannot write mode {0} as Palm".format(im.mode))
    ...
    fp.write(bytes((bpp,)))
    fp.write(bytes((version,)))
    fp.write(o16b(offset))
    fp.write(bytes((transparent_index,)))
    fp.write(bytes((compression_type,)))
    ...
    if colormapsize > 0:
        ...
        for i in range(256):
            fp.write(bytes((i,)))
            if colormapmode == 'RGB':
                fp.write(bytes((colormap[3 * i], colormap[3 * i + 1], colormap[3 * i + 2])))
            elif colormapmode == 'RGBA':
                fp.write(bytes((colormap[4 * i], colormap[4 * i + 1], colormap[4 * i + 2])))
